"""
每日泵开泵策略 — LSTM 流量预测 + PSO 寻优 融合脚本
=================================================

把 inference_lstm.py 与 pump_optimize_PSO.py 串成完整流水线:

  ① LSTM 流量预测 (inference_lstm.FlowPredictor)
       → 未来 96 个时间点 (15min × 24h) 的 总管流量 Total_Flow + 分时压力 Pressure
  ② 三管分流: LSTM 只预测总管流量, 170:1/170:2/70:3 的比例按最近
       ratio_lookback_days 天历史数据的"同一时段(15min槽位)"均值估计
       (70:3 深夜常为 0, 按时段比例比全天平均更贴近真实)
  ③ 逐点 PSO 寻优 (pump_optimize_PSO.optimize_strategy)
       → 每个时间点输出多条候选策略 (默认 top 5, 不同泵状态组合)
  ④ 全局策略选择 (动态规划): 对 96 个时间点的候选做全局路径寻优,
...
from inference_lstm import FlowPredictor, DEFAULT_DATA, DEFAULT_RESULT_DIR
...
FREQ_MINUTES = 15         # 与 LSTM 训练一致的重采样频率
POINTS_PER_DAY = (24 * 60) // FREQ_MINUTES   # 96


# ============================================================================
# ① 三管分流比例估计 (按 15min 时段槽位)
# ============================================================================

def compute_flow_ratios(df_raw, lookback_days, freq_minutes=FREQ_MINUTES):
    """由历史原始数据估计 170:1/170:2/70:3 三管分流比例。

    每根管独立做 15min 均值重采样, 取最近 lookback_days 天; 按天内的
    时段槽位 (0..95) 分组求多天均值, 得到每个槽位的三管比例。
    槽位缺失或总量为 0 时退回全天平均比例; 全天也为 0 时退回均分。

    返回 (global_ratio, slot_ratios):
      global_ratio : (3,) 全天平均比例
      slot_ratios  : (96, 3) 各 15min 槽位的比例 (行 i = 槽位 i)
    """
    df = df_raw.copy()
...
def select_global_path(cands_per_point, w_state, w_freq, w_eff, w_viol):
    """动态规划: 从每点的多条候选里选出 96 点全局代价最小的策略路径。
...
def main():
    ...
    # ── ① LSTM 流量预测 ──
    print("\n" + "=" * 72)
    print("① LSTM 流量预测 (inference_lstm.FlowPredictor)")
    print("=" * 72)
    predictor = FlowPredictor(args.result_dir)
    pred = predictor.predict(args.data)          # DataFrame: Total_Flow (96 行)
    pred = predictor.predict_pressure(pred)      # + Pressure 列 (分时压力目标)
    print(f"   预测点数: {len(pred)}  ({pred.index[0]} ~ {pred.index[-1]})")

    # ── ② 三管分流比例 ──
    print("\n" + "=" * 72)
    print(f"② 三管分流比例估计 (最近 {args.ratio_lookback_days} 天, 按 15min 时段)")
    print("=" * 72)
    ...
    if len(df_out) > 0:
        ...
        cur_start = df_out.iloc[0]["timestamp"]
        effs, kwts, feas = [], [], []
        prev_ts = cur_start
        ...
        for _, row in df_out.iterrows():
            if row["states"] == cur_states and row["freqs"] == cur_freqs:
                ...
                feas.append(row["feasible"]); prev_ts = row["timestamp"]
                continue
            # 切换组合: 前一段结束于本行前一个时刻
            end_ts = (pd.to_datetime(row["timestamp"])
                      - pd.Timedelta(minutes=FREQ_MINUTES)).strftime("%Y-%m-%d %H:%M:%S")
        ...
        if len(effs) > 0:
            _close_block(blocks, cur_start, prev_ts, len(effs), cur_states, cur_freqs,
                         effs, kwts, feas)
