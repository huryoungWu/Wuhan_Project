...
from train_transformer import BASE_CONFIG, DataProcessor, TimeSeriesTransformer

DEFAULT_DATA = r'D:\Wuhan_Project\input_lookback.csv'
DEFAULT_RESULT_DIR = r"D:\Wuhan_Project\results_transformer\L7_P24H_30min_transformer"
...
class FlowPredictor:
    """基于 train_transformer.py 训练结果的流量预测推理接口。
    ...
    def __init__(self, result_dir=DEFAULT_RESULT_DIR, device=None,
        ...
        print(f"[FlowPredictor] lookback={self.lookback_days}d ({self.lookback_steps}步), "
              ...
              f"nhead={self.config['nhead']}, layers={self.config['num_layers']}, "
    ...
    def predict(self, data, columns=None, encoding="utf-8-sig", as_list=False):
        ...
        pd.DataFrame 或 list[dict]
            DataFrame: index = 预测时刻 (30min 分辨率), 列 Total_Flow = 预测流量。
    ...
    def _predict_df(self, raw_df):
        ...
        # ── ③ Transformer Encoder 直接多步输出 (无自回归, 与测试评估一致) ──
        with torch.no_grad():
            pred = self.model(x_tensor, target_len=self.predict_steps)   # (1, pred, 1)
        # 反归一化 (训练若启用 target_transform=log1p, 此处会自动 expm1 还原)
        y_inv = self.processor.inverse_transform_targets(pred.cpu().numpy())[0]   # (pred, 1)
        ...
        print(f"[predict] 输出: {result.index[0]} ~ {result.index[-1]}, {len(result)} 行 ({self.resample_freq})")
        return result
