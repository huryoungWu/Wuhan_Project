...
# ============================================================================
# train_tcn.py — 基于 train_lstm.py 的模型架构升级版 (TCN 时序卷积网络)
#
# 数据清洗 / 特征工程 / 评估管线与 train_lstm.py 完全一致 (含按预测起点时刻统计),
# 仅替换模型: 原 BiLSTM Seq2Seq → TCN (膨胀因果卷积 + 残差块, Bai et al. 2018)。
#
# TCN 特性:
#   - 因果卷积保证不泄漏未来信息; 膨胀率 1,2,4,... 逐层翻倍,
#     感受野指数增长, 短堆叠即可覆盖整个回看窗口
#   - 直接多步输出 (无自回归 / 无 teacher forcing), 训练更稳定、并行度高
#   - 卷积局部感受野对流量突变 (泵切换阶跃等) 的捕捉通常优于循环模型
#
# 相对 train_lstm.py 的修改点:
#   1. BASE_CONFIG: 模型超参改为 tcn_channels / kernel_size / tcn_dropout
#   2. Encoder/Decoder/Seq2SeqModel → CausalConv1d / TemporalBlock / TCNForecast
...
BASE_CONFIG = {
    ...
    "label": "L7_P24H_15min_tcn",         # 结果子目录名
    ...
    # ── TCN 架构超参 (膨胀因果卷积) ──
    "tcn_channels": (64, 64, 64, 64, 64, 64),  # 每层通道数 (块数 = 通道数个数)
    "kernel_size": 5,                          # 卷积核大小 (局部感受野)
    "tcn_dropout": 0.2,                        # 块内 dropout (训练时)
    ...
    "base_result_dir": r"D:\Wuhan_Project\results_tcn",
}
...
class CausalConv1d(nn.Module):
    """因果膨胀卷积: 输出 t 时刻只依赖输入 t 及之前 (padding 只加在左侧)。

    膨胀因果卷积是 TCN 的核心: 不泄漏未来信息, 同时感受野随层数指数增长,
    少量堆叠即可覆盖整个回看窗口。
    """
    ...
    def forward(self, x):
        # padding 全加在左侧 (Conv1d 默认对称), 裁掉右侧等长输出 → 因果
        out = self.conv(x)
        if self.padding > 0:
...
class TemporalBlock(nn.Module):
    """TCN 残差块: 两层膨胀因果卷积 + ReLU (Bai et al. 2018)。

    残差连接缓解深层梯度消失; 通道数变化时用 1x1 卷积对齐。
    """
    def __init__(self, in_channels, out_channels, kernel_size, dilation, dropout):
        super().__init__()
        self.net = nn.Sequential(
            CausalConv1d(in_channels, out_channels, kernel_size, dilation, dropout),
            nn.ReLU(),
            CausalConv1d(out_channels, out_channels, kernel_size, dilation, dropout),
            nn.ReLU(),
        )
...
class TCNForecast(nn.Module):
    """TCN 多步预测模型: 膨胀因果卷积主干 + 线性输出头, 直接输出整个预测窗。
    ...
    def __init__(self, input_dim, output_dim, horizon, input_len,
                 channels=(64, 64, 64, 64, 64, 64), kernel_size=5, dropout=0.2):
        super().__init__()
        # 动态计算膨胀率: 感受野 = 1 + (k-1) * Σ 2^i 需 ≥ 回看窗口长度
        rf, dilations, depth = 1, [], 0
        while rf < input_len:
            dil = 2 ** depth
            rf += (kernel_size - 1) * dil
            dilations.append(dil)
            depth += 1
        channels = tuple(channels)
        # 配置通道数多于所需块数 → 补足膨胀率 (继续翻倍); 少于 → 复用最后一档通道数
        if len(dilations) < len(channels):
            base_dil = 2 ** len(dilations)
            dilations += [base_dil * (2 ** i) for i in range(len(channels) - len(dilations))]

        blocks = []
        n_blocks = len(dilations)
        for i in range(n_blocks):
            in_ch = input_dim if i == 0 else channels[min(i - 1, len(channels) - 1)]
            out_ch = channels[min(i, len(channels) - 1)]
            blocks.append(TemporalBlock(in_ch, out_ch, kernel_size, dilations[i], dropout))
        self.tcn = nn.Sequential(*blocks)
        self.receptive_field = rf

        self.output_dim = output_dim
        self.horizon = horizon
        # 取最后时刻的 TCN 表示 → 线性投影到整个预测窗 (直接多步)
        self.head = nn.Linear(channels[-1], horizon * output_dim)

    def forward(self, src, target_len, tgt=None, teacher_forcing_ratio=0.0):
        # Conv1d 通道在第二维: (B, L, F) → (B, C, L)
        h = self.tcn(src.transpose(1, 2))[:, :, -1]   # 最后时刻 (感受野覆盖全窗口)
...
def run_experiment(cfg, x_train_all, y_train_all, x_test_all, y_test_all, processor, device, test_index=None):
    ...
    print(f" 实验: {label}  |  lookback={lookback}d  |  predict={predict}d"
          ...
          f"  |  tcn_channels={cfg['tcn_channels']}  |  kernel={cfg['kernel_size']}"
    ...
    print(f"  TCN: 块数={len(cfg['tcn_channels'])}, 感受野={model.receptive_field} 步 "
          f"(≥ lookback {lookback_steps} 步)")
