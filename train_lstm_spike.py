...
# ============================================================================
# train_lstm_spike.py — 基于 train_lstm.py (train_lstm_clean_v2) 新增"突变毛刺"清洗
#
# 相对原版的主要修改 (对应缺陷分析):
...
#
# 相对 train_lstm_clean_v2 的新增修改 (本文件):
#   10. 新增"突变毛刺"清洗 detect_spikes: 基于一阶差分的稳健突变检测
#       (毛刺 = 突跳一格后立即回落的传感器瞬间干扰; 与 Hampel 互补——
#        Hampel 抓"值偏离局部水平", 本步抓"变化率突跳"),
#       检出置 NaN 后与 Hampel 结果一同走时间插值填补。
# ============================================================================
...
# ==================== 毛刺(突变)清洗参数 (新增) ====================
SPIKE_WINDOW  = 600     # 突变尺度统计窗口 (秒, 10 分钟; 滚动 MAD of 一阶差分)
SPIKE_K       = 6.0     # 突变阈值: |一阶差分| > k * scale 判为突变
SPIKE_CONFIRM = True    # 用后一点确认"立即回落" → 区分毛刺与真实阶跃起点
...
def detect_spikes(s, window=SPIKE_WINDOW, k=SPIKE_K, floor_ratio=MAD_FLOOR_RATIO,
                  confirm=SPIKE_CONFIRM, guard=None):
    """突变毛刺检测: 基于一阶差分的稳健跳变检测。

    毛刺定义: 数据突跳一格后立即回落 (传感器瞬间干扰/采集毛刺)。
    与 detect_outliers (Hampel) 互补: Hampel 抓"值偏离局部水平", 本函数
    抓"变化率突跳"——平稳段偶发单点跳变时, 跳变幅度未必超过 Hampel 的
    k*scale, 但一阶差分判据直接命中。

    判据 (逐点):
      1. |Δ_t| = |s_t - s_{t-1}| > k * scale              → 突跳
      2. scale = max(1.4826 * MAD(Δ窗口), floor_ratio * |局部中位数|)
         —— 差分尺度: 序列缓慢漂移时 Δ 稳定偏大但不"突变", 不会误报;
            底部分数保证信号极稳定 (MAD≈0) 时正常波动不误报
      3. confirm=True: 后一点回到起点附近
         |s_{t+1} - s_{t-1}| < |Δ_t| / 2                  → 确认是毛刺
         而非真实阶跃的起点 (真实阶跃会持续, 不会立即回落)。
         用后一点只为"不误伤", 与 pump_guard 同理 (见行内注释),
         不向特征注入信息; 清洗本就在训练/测试切分后分段执行, 无边界泄漏。
      4. guard 掩码 (泵切换 ±5min) 内不判定 (泵切换是真实阶跃)

    Returns
    -------
    flag : pd.Series[bool]  — True = 毛刺, 置 NaN 后由下方插值填补
    """
    d = s.diff()
    med = s.rolling(window, center=False, min_periods=window // 2).median()
    d_mad = d.abs().rolling(window, center=False, min_periods=window // 2).median()
    scale = np.maximum(1.4826 * d_mad, floor_ratio * med.abs())
    jump = d.abs() > k * scale
    if confirm:
        # 毛刺形状: s_{t+1} 回到 s_{t-1} 附近 (跳变是暂时的, 不是持续阶跃)
        ret = (s.shift(-1) - s.shift(1)).abs() < d.abs() / 2
        jump = jump & ret
    flag = jump.fillna(False)
    if guard is not None:
        flag = flag & ~guard
    return flag
...
class DataProcessor:
    ...
    def clean_and_resample(self, df):
        ...
        新增突变毛刺清洗 (detect_spikes, 一阶差分判据) 先于 Hampel 执行;
        ...
        # ── 突变毛刺清洗 (新增): 先于 Hampel, 检出后置 NaN, 同样由下方插值填补 ──
        # 毛刺 = 突跳一格后立即回落的瞬间干扰; Hampel 抓"偏离水平", 本步抓"变化率突跳"
        for col in hampel_targets:
            if col not in out.columns:
                continue
            flag = detect_spikes(out[col], guard=pump_guard)
            n_sp = int(flag.sum())
            if n_sp > 0:
                out.loc[flag, col] = np.nan
                print(f"  {col} 突变毛刺 {n_sp} 条 ({n_sp / len(out):.3%}) 置 NaN 待插值")
