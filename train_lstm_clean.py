...
# ============================================================================
# train_lstm_clean_v2.py — 基于 train_lstm.py 的数据清洗缺陷修复版
#
# 相对原版的主要修改 (对应缺陷分析):
...
EXPERIMENTS = [
    ...
    # {"lookback_days": 1,  "predict_days": 1.0,  "resample_freq": "15min",  "test_days": 10,  "dropout": 0.1, "hidden_dim": 64, "label": "L7_P24H_5min_clean_1"},
    {"lookback_days": 1,  "predict_days": 1.0,  "resample_freq": "15min",  "test_days": 10,  "dropout": 0.1, "hidden_dim": 64, "label": "L7_P24H_5min_clean_2"},
    {"lookback_days": 1,  "predict_days": 1.0,  "resample_freq": "15min",  "test_days": 10,  "dropout": 0.1, "hidden_dim": 64, "label": "L7_P24H_5min_clean_4"},
    {"lookback_days": 1,  "predict_days": 1.0,  "resample_freq": "15min",  "test_days": 10,  "dropout": 0.1, "hidden_dim": 64, "label": "L7_P24H_5min_clean_5"},
    {"lookback_days": 1,  "predict_days": 1.0,  "resample_freq": "15min",  "test_days": 10,  "dropout": 0.1, "hidden_dim": 64, "label": "L7_P24H_5min_clean_6"}
...
class DataProcessor:
    ...
    def build_feature_table(self):
        ...
        # 边界取本频率组内所有实验 test_days 的最小值, 保证任何实验的切分点都不落在
        # 同一次清洗段内部。滞后/滚动特征仍在拼接后的全量表上计算 (因果特征, 无泄漏)。
        test_days = self.config.get("clean_test_days", self.config.get("test_days", 15))
...
def main():
    ...
    for freq, group in groups.items():
        ...
        # 缺陷3修复: 清洗分段边界取本组最小 test_days, 保证任何实验的切分都不跨清洗段
        min_test_days = min(e.get("test_days", base_config["test_days"]) for e in group)
        group_cfg["clean_test_days"] = min_test_days
