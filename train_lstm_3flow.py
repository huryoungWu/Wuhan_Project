...
# ============================================================================
# train_lstm_3flow.py — 基于 train_lstm.py (clean_v2), 只预测第三分支 70:3 的流量
#
# 相对原版的主要修改 (对应缺陷分析):
#   1. 重采样后先 dropna 删除空 bin, 不再 ffill/bfill 常数填充
#      (原版 ffill().bfill().dropna() 中 dropna 是死代码, 空洞被填成常数平台)
#   2. 清洗顺序改为"逐管清洗→再求和": 每根管独立做 Hampel/界限裁剪/插值,
#      求和要求三管都有效 (min_count=3), 任一管缺失则总量置 NaN → 空 bin 被删除,
#      不再静默低估总流量 (原版 sum(skipna=True) 会把单管掉线当成真实低流量)
#   3. 清洗/插值在"按时间切分之后"分段执行 (build_feature_table 内先切分再清洗):
#      插值不跨越训练/测试边界; Hampel 改 center=False 只使用过去数据 → 消除边界泄漏
#   4. 泵切换保护: 运行泵数量变化的 ±5 分钟内不判异常 (泵切换是真实阶跃, 不该被抹平)
#   5. 泵列全缺失 → 运行泵数量 = NaN 而不是 0 (缺失≠停泵); 泵数量是状态量, 不做插值
#   6. 压力列补做 Hampel 异常检测 (原版压力无任何异常检测)
#   7. 插值设上限 MAX_IMPUTE_ROWS: 超过 15 分钟的空洞不插值, 保留 NaN 交给空 bin 删除
#   8. 泵相关 lag 改为按分钟折算 (原版 shift(1)/shift(6) 硬编码步数, 换频率后物理含义漂移)
#   9. 新增累计流量 (LJflowtotal) 交叉校验: 打印统计, 不改数据
#
# 注意: 本版本与 plot_daily_flow_curves.py 的 Hampel 不再完全一致
#       (原版 center=True 使用未来数据; 本版 center=False 因果化)
#
# 相对 train_lstm.py (train_lstm_clean_v2) 的新增修改 (本文件):
#   10. 预测目标改为单分支 70:3 的瞬时流量 (TARGET_COL), 不再预测总流量:
#       - DataProcessor.target_cols → [TARGET_COL]; 只预测第三分支, 170:1/170:2 不预测
#       - 三根管仍全部参与清洗与特征工程 (Total_Flow 特征需要三管求和;
#         170:1/170:2 的原始流量也作为特征输入), 只是不作为预测目标
#       - 逐管清洗后的分流量原样重采样保留 (70:3 为标签列, 其余为特征)
#       - 输出维度 1; MAPE 只统计 |真值| ≥ MAPE_FLOOR 的点 (70:3 夜间/停泵近零)
#       - 注意: 推理脚本 inference_lstm.py 只支持 1 维输出, 本模型可直接复用,
#         但结果目录/特征列与原版不同 (scaler.pkl 已存 target_cols)
# ============================================================================
...
# ==================== 预测目标配置 (单分支版) ====================
# 预测目标: 只预测第三分支 70:3 的瞬时流量 (原版预测三管之和 Total_Flow)。
# 三根管仍全部参与清洗/特征 (Total_Flow 特征 = 三管求和; 170:1/170:2
# 原始流量也作为特征输入模型), 只是不作为预测目标。
FLOW_COLS = ["170:1_瞬时流量", "170:2_瞬时流量", "70:3_瞬时流量"]   # 清洗/特征用 (三管)
TARGET_COL = "70:3_瞬时流量"                                       # 预测目标 (第三分支)
MAPE_FLOOR = 10.0   # m³/h: MAPE 只统计 |真值| ≥ 此值的点 (70:3 停泵/夜间近零, 相对误差会爆炸)
...
BASE_CONFIG = {
    ...
    "label": "L7_P24H_15min_pipe3",   # 结果子目录名 (单分支版独立目录, 不覆盖原版结果)
    ...
    "base_result_dir": r"D:\Wuhan_Project\results_lstm_seq",
}
...
class DataProcessor:
    def __init__(self, config):
        ...
        # 单分支版: 预测目标 = 第三分支 70:3; 总流量只作特征, 不再作目标
    ...
    def build_base_features(self, df):
        ...
        """
        # 武汉数据: 三根分流管 = 170:1 + 170:2 + 70:3 (无总管流量列, 不含 70:7)
    ...
    def clean_and_resample(self, df):
        ...
        # 单分支版: 逐管清洗后的分流量原样重采样保留 (70:3 为标签列, 其余为特征)
        for col in flow_cols:
            res[col] = out[col].resample(freq).mean()
...
def weighted_mse_loss(pred, target, flow_weight=1.0):
    # 单分支版: 对全部输出通道求 MSE (目标只含 70:3 一路)
    flow_loss = ((pred - target) ** 2).mean()
...
def evaluate(model, loader, device, processor):
    ...
    # MAPE 修正 (3流版专属问题): 分流量在停泵/夜间会接近 0, 除以极小真值
    # 使相对误差爆炸 (总流量版不会出现——总量始终为正)。只对 |真值| ≥ MAPE_FLOOR
    # 的点统计, 同时报告覆盖率, 避免掩盖近零段占比。
    mask = np.abs(y_true_flat) >= MAPE_FLOOR
    flow_mape = np.mean(np.abs((y_true_flat[mask] - y_pred_flat[mask])
                               / (y_true_flat[mask] + 1e-8))) * 100
    mape_coverage = mask.mean() * 100

    # 单分支版: 目标列单独 MAE (现只有 70:3 一列)
    pipe_mae = {}
    for i, c in enumerate(processor.target_cols):
        pipe_mae[f"mae_{c}"] = mean_absolute_error(
            y_true_inv[:, :, i].reshape(-1), y_pred_inv[:, :, i].reshape(-1))

    return {
        "loss": avg_loss, "flow_mae": flow_mae, "flow_rmse": flow_rmse,
        "flow_mape": flow_mape, "mape_coverage": mape_coverage, "pipe_mae": pipe_mae,
        "y_pred_inv": y_pred_inv, "y_true_inv": y_true_inv
    }


def plot_best_worst_cases(y_true_inv, y_pred_inv, save_dir, num_best=30, num_worst=30,
                          title_prefix="Test", target_cols=None):
    n_samples = len(y_true_inv)
    if n_samples == 0:
        return

    per_sample_mae = np.mean(np.abs(y_true_inv - y_pred_inv), axis=(1, 2))
    # 单分支版: 每个样本的目标曲线 真值/预测 (目标只有 70:3, n_pipe=1)
    n_pipe = y_true_inv.shape[2] if y_true_inv.ndim == 3 else 1
    pipe_labels = (list(target_cols) if target_cols is not None
                   else [f"管{i+1}" for i in range(n_pipe)])
    pipe_colors = ["#2c3e50", "#16a085", "#8e44ad", "#d35400", "#27ae60"]
    pipe_pred_colors = ["#e74c3c", "#e67e22", "#2980b9", "#c0392b", "#1abc9c"]
    ...
    def draw_grid(indices, nrows, tag, save_filename):
        ...
        for i, idx in enumerate(indices):
            ...
            for p in range(n_pipe):
                ax.plot(y_true_inv[idx, :, p], color=pipe_colors[p], linewidth=1.2,
                        label=f'{pipe_labels[p]}真' if r == 0 and c == 0 else None)
                ax.plot(y_pred_inv[idx, :, p], color=pipe_pred_colors[p], linewidth=1.2,
                        linestyle='--', label=f'{pipe_labels[p]}预' if r == 0 and c == 0 else None)
...
def run_experiment(cfg, x_train_all, y_train_all, x_test_all, y_test_all, processor, device):
    ...
    model = Seq2SeqModel(
        ...
        dropout=cfg["dropout"],
        output_dim=len(processor.target_cols)   # 单分支版: 输出 70:3 一路
    ).to(device)
    ...
    print(f"  Train: Loss={train_metrics['loss']:.6f}, MAE={train_metrics['flow_mae']:.2f}, "
          ...
          f"(覆盖 {train_metrics['mape_coverage']:.1f}% 点)")
    print(f"  Test : Loss={test_metrics['loss']:.6f}, MAE={test_metrics['flow_mae']:.2f}, "
          ...
          f"(覆盖 {test_metrics['mape_coverage']:.1f}% 点)")

    # 单分支版: 目标列 MAE 明细
    for name, m in [("Train", train_metrics), ("Test", test_metrics)]:
        pipe_str = ", ".join(f"{c}={m['pipe_mae'][f'mae_{c}']:.2f}"
                             for c in processor.target_cols)
        print(f"  {name} 目标MAE: {pipe_str}")
    ...
    if len(y_true_test) > 0:
        plot_best_worst_cases(y_true_test, y_pred_test, result_dir, num_best=30, num_worst=30,
                              title_prefix="Test", target_cols=processor.target_cols)
        plot_error_distribution(y_true_test, y_pred_test,
                                os.path.join(result_dir, "test_error_distribution.png"), title_prefix="Test")

        # 单分支版: 分步 MAE 按目标列输出 (现只有 70:3 一列)
        mae_per_step = np.mean(np.abs(y_true_test - y_pred_test), axis=0)   # (steps, n_pipe)
        step_mae_df = pd.DataFrame({"step": np.arange(len(mae_per_step))})
        for i, c in enumerate(processor.target_cols):
            step_mae_df[f"mae_{c}"] = mae_per_step[:, i]
        step_mae_df["mae_avg"] = mae_per_step.mean(axis=1)
    ...
    return {
        ...
        "test_pipe_mae": test_metrics["pipe_mae"],
    }
